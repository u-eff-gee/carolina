    ...
    while(getline(ifile, line)){
        if(line.find("@REGISTER_BRANCHES@") != std::string::npos){
            ...
            for(size_t n_d_0 = 0; n_d_0 < detectors.size()-1; ++n_d_0){
                ofile << "\t\tif(";
                ofile << energy_deposition_in_detector(detectors[n_d_0]);
                ofile << "){\n";

                for(size_t n_d_1 = n_d_0 + 1; n_d_1 < detectors.size(); ++n_d_1){
                    ofile << "\t\t\tif(";
                    ofile << energy_deposition_in_detector(detectors[n_d_1]);
                    ofile << "){\n";
                    ...
                    if(detectors[n_d_0].channels.size() > 1){
                        ...
                        for(size_t n_c_0 = 0; n_c_0 < detectors[n_d_0].channels.size(); ++n_c_0){
                            ofile << "\t\t\t\t" << detectors[n_d_0].name << "_energies[" << to_string(n_c_0) << "] = " << calibrate_if_not_nan(detectors[n_d_0].channels[n_c_0].energy_branch_name + "[" + to_string(detectors[n_d_0].channels[n_c_0].energy_branch_index) + "]", detectors[n_d_0].channels[n_c_0].energy_calibration_parameters) << ";\n";
                            if(n_c_0 > 0){
                        ...
                        ofile << "\t\t\t\tenergy_0 = " << sum_over_array(detectors[n_d_0].name + "_energies", detectors[n_d_0].channels.size()) << ";\n";
                    } else{
                        ofile << "\t\t\t\tenergy_0 = " << calibration_expression(detectors[n_d_0].channels[0].energy_branch_name + "[" + to_string(detectors[n_d_0].channels[0].energy_branch_index) + "]", detectors[n_d_0].channels[0].energy_calibration_parameters) << ";\n";
                        ofile << "\t\t\t\ttime_0 = " << detectors[n_d_0].channels[0].time_branch_name << "[" << detectors[n_d_0].channels[0].time_branch_index << "];\n";
                    }
                    ...
                    if(detectors[n_d_1].channels.size() > 1){
                        ...
                        for(size_t n_c_1 = 0; n_c_1 < detectors[n_d_1].channels.size(); ++n_c_1){
                            ofile << "\t\t\t\t" << detectors[n_d_1].name << "_energies[" << to_string(n_c_1) << "] = " << calibrate_if_not_nan(detectors[n_d_1].channels[n_c_1].energy_branch_name + "[" + to_string(detectors[n_d_1].channels[n_c_1].energy_branch_index) + "]", detectors[n_d_1].channels[n_c_1].energy_calibration_parameters) << ";\n";
                            if(n_c_1 > 0){
                        ...
                        ofile << "\t\t\t\t\tenergy_1 = " << calibration_expression(detectors[n_d_1].channels[0].energy_branch_name + "[" + to_string(detectors[n_d_1].channels[0].energy_branch_index) + "]", detectors[n_d_1].channels[0].energy_calibration_parameters) << ";\n";
                        ofile << "\t\t\t\ttime_1 = " << detectors[n_d_1].channels[0].time_branch_name << "[" << detectors[n_d_1].channels[0].time_branch_index << "];\n";
                    }

                    histogram_name = coincidence_histogram_name(detector_groups, detectors[n_d_0], detectors[n_d_1]);

                    ofile << "\t\t\t\tcoincidence_" << histogram_name << "->Fill(energy_0, energy_1);\n";
                    ofile << "\t\t\t\ttime_difference_" << histogram_name << "->Fill(" << calibration_expression("time_0", detectors[n_d_0].channels[0].time_calibration_parameters) << " - ( " << calibration_expression("time_1", detectors[n_d_1].channels[0].time_calibration_parameters) << "));\n";
                    if(detectors[n_d_0].group.name == detectors[n_d_1].group.name){
                        ofile << "\t\t\t\tcoincidence_" << histogram_name << "->Fill(energy_1, energy_0);\n";
                        ofile << "\t\t\t\ttime_difference_" << histogram_name << "->Fill(" << calibration_expression("time_1", detectors[n_d_1].channels[0].time_calibration_parameters) << " - ( " << calibration_expression("time_0", detectors[n_d_0].channels[0].time_calibration_parameters) << "));\n";
                    }
