...
int main(int argc, char *argv[]) {
    ...
    po::options_description desc(
        "Create a shell script to run an analysis script on a ROOT TTree in "
        "multitasked mode, using screen or slurm. As compared to true "
        "multithreading, multitasking means that the same analysis script will "
        "be launched multiple times to work on subsets of the TTree's entries. "
        "In particular, each instance of the analysis script will use its own "
        "memory.");
    desc.add_options()("help", "Produce help message.")(
        "tool", po::value<string>()->default_value("screen"),
        "Tool for multitasking [options: 'screen' (default), slurm].")(
        "input_file", po::value<vector<string>>(), "Input file name.")(
        "n_tasks", po::value<unsigned int>()->default_value(1),
        "Number of tasks (default: 1).")(
        "script", po::value<string>()->default_value("histograms_1d"),
        "Name of the script to be run in multitasking mode (default: "
        "'histograms_1d').")(
        ...
        "tool).")("tree", po::value<string>()->default_value("clover_array"),
    ...
    if (vm["tool"].as<string>() == "screen") {
        ...
        cout << "Unrecognized tool '" << vm["tool"].as<string>()
             << "'. The available options are 'screen' or 'slurm'. Aborting ..."
             << endl;
        return 1;
    }
    ...
    if (tool == "screen") {
        ...
        for (unsigned int i = 0; i < n_tasks; ++i) {
            ...
            output_file << "--tree " << vm["tree"].as<string>() << " "
                        << "--first " << i * increment + 1 << " "
                        << "--last "
                        << (i == n_tasks - 1 ? (i + 1) * increment + modulo
                                             : (i + 1) * increment)
                        << " "
        ...
        cout << "Generated shell script '" << output_file_name
             << "' which runs '" << vm["script"].as<string>() << "' on "
             << n_tasks << " tasks." << endl;
        ...
        output_file << "bash multitask_"
                    << vm["script"].as<string>() + "_" + tool +
                           "_${SLURM_ARRAY_TASK_ID}.sh"
                    << "\n";
        output_file.close();
        cout << "Generated shell script '" << output_file_name
             << "' which runs '" << vm["script"].as<string>() << "' on "
        ...
        cout << "You may have to make the shell script executable first:"
             << endl;
        cout << "\tchmod +x " << output_file_name << endl;
        cout << "The shell script will run the following input files, which "
                "represent multiple calls to '"
             << vm["script"].as<string>() << ":" << endl;

        for (unsigned int i = 0; i < n_tasks; ++i) {
            output_file_name = "multitask_" + vm["script"].as<string>() + "_" +
                               tool + "_" + to_string(i) + ".sh";
            ofstream output_file(output_file_name);
            output_file << "./";
            output_file << vm["script"].as<string>() << " ";
            for (auto input_file : input_files) {
