...
size_t coincidence_histogram_index(const DetectorGroup group_0,
                                   const DetectorGroup group_1) {
    size_t index = 0;
    for (size_t n_g_0 = 0; n_g_0 < groups.size(); ++n_g_0) {
        for (size_t n_g_1 = n_g_0; n_g_1 < groups.size(); ++n_g_1) {
            if ((group_0.name == groups[n_g_0].name &&
                 group_1.name == groups[n_g_1].name) ||
                (group_1.name == groups[n_g_0].name &&
                 group_0.name == groups[n_g_1].name)) {
                return index;
            }
            ++index;
        }
...
HistogramProperties
common_time_difference_histogram(const DetectorGroup group_0,
                                 const DetectorGroup group_1) {
    if (group_0.time_difference_histogram_properties.n_bins !=
        group_1.time_difference_histogram_properties.n_bins) {
        cout << "Warning: Different number of bins in time-difference "
                "histograms of detector groups '"
             << group_0.name << "' ("
             << group_0.time_difference_histogram_properties.n_bins << ") and '"
             << group_1.name << " ("
             << group_1.time_difference_histogram_properties.n_bins
             << "). Selecting the larger number." << endl;
    }
    if (group_0.time_difference_histogram_properties.minimum !=
        group_1.time_difference_histogram_properties.minimum) {
        cout << "Warning: Different lower limit in time-difference histograms "
                "of detector groups '"
             << group_0.name << "' ("
             << group_0.time_difference_histogram_properties.minimum
             << ") and '" << group_1.name << " ("
             << group_1.time_difference_histogram_properties.minimum
             << "). Selecting the lower limit." << endl;
    }
    if (group_0.time_difference_histogram_properties.maximum !=
        group_1.time_difference_histogram_properties.maximum) {
        cout << "Warning: Different upper limit in time-difference histograms "
                "of detector groups '"
             << group_0.name << "' ("
             << group_0.time_difference_histogram_properties.maximum
             << ") and '" << group_1.name << " ("
             << group_1.time_difference_histogram_properties.maximum
             << "). Selecting the higher limit." << endl;
    }

    return {max(group_0.time_difference_histogram_properties.n_bins,
                group_1.time_difference_histogram_properties.n_bins),
            min(group_0.time_difference_histogram_properties.minimum,
                group_1.time_difference_histogram_properties.minimum),
            max(group_0.time_difference_histogram_properties.maximum,
                group_1.time_difference_histogram_properties.maximum)};
}
...
int main(int argc, char **argv) {
    ...
    for (size_t n_g_0 = 0; n_g_0 < groups.size(); ++n_g_0) {
        for (size_t n_g_1 = n_g_0; n_g_1 < groups.size(); ++n_g_1) {
            ...
            HistogramProperties time_difference_histogram_properties =
                common_time_difference_histogram(groups[n_g_0], groups[n_g_1]);
    ...
    for (int i = first; i <= last; ++i) {
        ...
        for (size_t n_d_0 = 0; n_d_0 < detectors.size(); ++n_d_0) {
            if (detectors[n_d_0].channels.size() > 1) {
                for (size_t n_c_0 = 0; n_c_0 < detectors[n_d_0].channels.size();
                     ++n_c_0) {
                    if (detectors[n_d_0].addback_energies[n_c_0] != 0.) {
                        for (size_t n_d_1 = n_d_0 + 1; n_d_1 < detectors.size();
                             ++n_d_1) {
                            if (detectors[n_d_1].channels.size() > 1) {
                                for (size_t n_c_1 = 0;
                                     n_c_1 < detectors[n_d_1].channels.size();
                                     ++n_c_1) {
                                    if (detectors[n_d_1]
                                                .addback_energies[n_c_1] !=
                                            0. &&
                                        fabs(detectors[n_d_1]
                                                 .addback_times[n_c_1] -
                                             detectors[n_d_0]
                                                 .addback_times[n_c_0]) <=
                                            max(detectors[n_d_0]
                                                ...
                                                detectors[n_d_1]
                                                    ...
                                                    .coincidence_window_half_width)) {
                                        histogram_index =
                                            coincidence_histogram_index(
                                                detectors[n_d_0].group,
                                                detectors[n_d_1].group);
                                        coincidence_histograms[histogram_index]
                                            ->Fill(
                                                detectors[n_d_0]
                                ...
                                if (detectors[n_d_1]
                                    ...
                                    histogram_index =
                                        coincidence_histogram_index(
                                            ...
                                            detectors[n_d_1].group);
                                    coincidence_histograms[histogram_index]
                                        ->Fill(detectors[n_d_0]
                                                   .addback_energies[n_c_0],
                                               detectors[n_d_1]
                                                   .channels[0]
                                                   .energy_calibrated);
                                    time_difference_histograms[histogram_index]
                                        ->Fill(detectors[n_d_0]
                                                   .addback_times[n_c_0] -
                                               detectors[n_d_1]
                                                   .channels[0]
                    ...
                    for (size_t n_d_1 = n_d_0 + 1; n_d_1 < detectors.size();
                        ...
                        if (detectors[n_d_1].channels.size() > 1) {
                            for (size_t n_c_1 = 0;
                                 n_c_1 < detectors[n_d_1].channels.size();
                                 ++n_c_1) {
                                if (detectors[n_d_1].addback_energies[n_c_1] !=
                                        0. &&
                                    fabs(detectors[n_d_1].addback_times[n_c_1] -
                                         detectors[n_d_0]
                                             .channels[0]
                                             .time_calibrated) <=
                                        max(detectors[n_d_0]
                                ...
                                fabs(detectors[n_d_1]
                                    ...
                                    max(detectors[n_d_0]
                                        ...
                                        detectors[n_d_1]
                                            .group
                                            .coincidence_window_half_width)) {
                                histogram_index = coincidence_histogram_index(
                                    detectors[n_d_0].group,
                                    detectors[n_d_1].group);
                                coincidence_histograms[histogram_index]->Fill(
                                    detectors[n_d_0]
                                        .channels[0]
                                        .energy_calibrated,
                                    detectors[n_d_1]
